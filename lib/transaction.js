...
function Transaction(createQuery) {
  ...
  StateMachine.call(this, 'pending', {
    ...
    'pending':    [ 'opening' ],
    'opening':    [ 'dequeueing' ],
    'dequeueing': [ 'open', 'rollback:complete', 'committed' ],
...
Transaction.prototype.begin = function (conn, callback) {
  ...
  conn.query('begin', function (err) {
    ...
    var unsubErrors = conn.removeListener.bind(conn, 'error', self.handleError)
    self.once('rolled back', unsubErrors)
    self.once('committed', unsubErrors)
