...
var QueryProxy = require('./query-proxy')
...
Adapter.prototype._execute = function (query) {
	...
	if (!query._callback) {
		var eventHandlers = this.queryEventHandlers
		for (var evt in eventHandlers) {
			query.proxyEvent(driverQuery, evt, eventHandlers[evt])
		}
	} else {
		// Always proxy 'end' events
		query.proxyEvent(driverQuery, 'end')
	}
