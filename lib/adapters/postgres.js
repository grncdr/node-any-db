...
Postgres.prototype.query = helpers.queryMethod(function (stmt, params, callback, qa) {
	var args
		, qa = qa || new QueryAdapter
		, receivedRow = false
	  , pgResult;

	try {
		var i = 0;
		function nextPlaceholder () { return '$' + (++i) }
		args = helpers.fixPlaceholders(stmt, params, nextPlaceholder)
	} catch (err) {
	...
	var query = this._connection.query(args[0], args[1])
